        ...
        <div className="h-full flex flex-col bg-gray-100 font-sans">
            ...
            >
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-xl border shadow-sm transition-all duration-300 min-w-[200px]"
                        style={{ borderColor: `${props.teams.find(t => t.id === props.currentTeamId)?.color || '#3b82f6'}40` }}
                    >
                        <Users size={16} style={{ color: props.teams.find(t => t.id === props.currentTeamId)?.color || '#3b82f6' }} />
                        <select
                            className="bg-transparent text-sm font-bold focus:outline-none text-gray-700 w-full cursor-pointer"
                            value={props.currentTeamId ?? 0}
                            onChange={(e) => {
...
const OpgaverPage: React.FC = () => {
    const { state, setCurrentTeamId } = useAppState();
    const { users } = state;
    ...
    const fetchTasks = async () => {
        ...
        try {
            const data = await opgaveService.getAll({
                team: state.currentTeamId || undefined
            });
            setTasks(data);
        } catch (error) {
            console.error(error);
