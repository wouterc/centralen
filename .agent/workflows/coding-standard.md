...
3. **Audit for Localization**: If modifying frontend UI, ensure all new text uses the `t()` hook from `useTranslation`. Always use **English** as the fallback default string in the hook (e.g. `t('key', 'English default')`).
...
- "Is there any hardcoded Danish text in the JSX, or did I forget to use English as the default string in t('key', 'English default')?"
